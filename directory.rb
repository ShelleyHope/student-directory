...
# 		elsif cohort.downcase.chars[0]=="f" then cohort = :Feb
# 		end
...
def print_list(people)
	i = 0
		...
		# print "#{i += 1}. #{person[:name].capitalize} \t\t#{person[:age]} 
		# \t\t#{person[:country_of_birth].capitalize} \t\t(#{person[:cohort]} Cohort)\n"
		printf "#{i += 1}. #{person[:name]} \t\t(#{person[:cohort]} Cohort)\n"
		end
		# end
end
...
def Feb_cohort(people)
	return people.find_all {|person| person[:cohort] == :Feb}
	# return Feb_array
	end
def	March_cohort(people)
	return people.find_all {|person| person[:cohort] == :March}
	# return Mar_array
end


students = [{:name => "Adam",:cohort => :Feb},{:name => "Anthony",:cohort => :Feb},{:name => "Peter",:cohort => :Feb},
	{:name => "Robin",:cohort => :March},{:name => "Bethsheeba",:cohort => :March},{:name => "Marcus",:cohort => :March}]

#print_footer(students)
#puts students
# print_list(students)

Feb_array = Feb_cohort(students)
Mar_array = March_cohort(students)
print_list(Feb_array)
print_list(Mar_array)
